...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    var target = self.options.entryPoint;//url.parse(self.options.entryPoint, true);

    if (operation === 'logout') {
      if (self.options.logoutUrl) {
        target = self.options.logoutUrl;//url.parse(self.options.logoutUrl, true);
      }
    ...
    var samlMessage = request ? {
      SAMLRequest: encodeURIComponent(base64)
    } : {
      SAMLResponse: encodeURIComponent(base64)
    };
