...
SAML.prototype.signRequest = function (samlMessage) {
  ...
  switch(this.options.signatureAlgorithm) {
    case 'sha256':
      ...
      break;
    default:
      samlMessage.SigAlg = encodeURIComponent("http://www.w3.org/2000/09/xmldsig#rsa-sha1");//'http://www.w3.org/2000/09/xmldsig#rsa-sha1';
      signer = crypto.createSign('RSA-SHA1');
      break;
  }
  if (samlMessage.SAMLRequest) {
    samlMessageToSign.SAMLRequest = encodeURIComponent(samlMessage.SAMLRequest);
  }
  ...
  samlMessage.Signature = encodeURIComponent(signer.sign(this.options.privateCert, 'base64'));
};
...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    if (self.options.privateCert && operation === 'logout') {
        ...
        var encodedRelayState = "https://localdev-leapfrog.mercer.com:3100/";

        // TBD. We should really include a proper RelayState here
        if (self.options.privateCert) {
            var strSignature = "SAMLRequest=" + encodedRequest;
            strSignature += "&RelayState=" + encodedRelayState;
            strSignature += "&SigAlg=" + encodedSigAlg;

            var signer = crypto.createSign('RSA-SHA1');
            signer.update(strSignature);
            var signature = signer.sign(self.options.privateCert, 'base64');

            var encodedSignature = encodeURIComponent(signature);
            finalSignatureValue = "&Signature=" + encodedSignature + "&SigAlg=" + encodedSigAlg;

            target += "SAMLRequest=" + encodedRequest;
